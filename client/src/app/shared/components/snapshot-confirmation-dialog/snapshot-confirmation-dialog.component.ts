...
export class SnapshotConfirmationDialogComponent {
  ...
  onConfirm() {
    ...
    this.apiService
      .checkAmiLimit(
        this.data.instanceElement.ec2InstanceId,
        this.data.instanceElement.availabilityZone.slice(0, -1),
      )
      .pipe(
        switchMap((response) => {
            ...
            return dialogRef.afterClosed().pipe(
              switchMap((confirmed) => {
                ...
                return this.apiService
                  .deleteInstanceAmi(
                    this.data.instanceElement.deploymentId,
                    this.data.instanceElement.availabilityZone.slice(0, -1),
                    response.oldest_image_id,
                  )
                  .pipe(
                    switchMap(() =>
